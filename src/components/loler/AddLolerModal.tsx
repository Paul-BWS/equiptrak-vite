...
export function AddLolerModal({ customerId }: AddLolerModalProps) {
  ...
  React.useEffect(() => {
    if (certificateNumber) {
      form.setValue("certificate_number", certificateNumber);
    }
  }, [certificateNumber, form]);

  // Calculate retest date based on test date
  const retestDate = form.watch("test_date") 
    ? format(addDays(new Date(form.watch("test_date")), 364), "dd/MM/yyyy")
    : "";
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      console.error("Error creating LOLER equipment:", error);
      toast({
        title: "Error",
